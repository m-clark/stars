...
Workshops see all types of folks at different levels. This is for some of the more technically inclined, though you'll find more context and detail in [my doc](https://m-clark.github.io/docs/GAM.html), at least which some of the following is lifted straight from.

## A detailed example

The following will demonstrate a polynomial spline by hand. It is based on a depiction in Fahrmeier et al. (2013).
...
Above, each $B_j$ is a <span class="emph">[basis function](https://en.wikipedia.org/wiki/Basis_function)</span> that is the transformed $X$ depending on the type of basis considered, and the $\gamma$ are the corresponding regression coefficients.  It looks complicated, but you've done this before.  Let's go back to the quadratic polynomial, there the basis function includes an intercept, x and x^2^.
...
Let's compare it to the <span class='func'>gam</span> function from the <span class='pack'>mgcv</span> package. As we'll see later we won't usually specify the knots directly, and even as we have set things up similar very close to our own approach, the <span class="func">gam</span> function is still doing some things our by-hand approach is not (penalized regression).  We still get pretty close agreement.
...
Consider a standard GLM that we usually estimate with maximum likelihood, $l(\beta)$, where $\beta$ are the associated regression coefficients. *Conceptually* we can write the <span class="emph">penalized likelihood</span> as follows:


$$l_p(\beta)= l(\beta) - \color{darkred}{\frac{1}{2}\lambda\sum\beta^2}$$

If you prefer least squares as the loss function, we can put it as:

$$\mathcal{Loss} = \sum (y-X\beta)^2 + \color{darkred}{\frac{1}{2}\lambda\sum\beta^2}$$

So if we're maximizing the likelihood, the penalty will make it lower than it otherwise would have been, and vice versa in terms of a loss function.  What it means for a GAM is that we'll add a penalty for the coefficients associated with the (non-linear) basis functions (e.g. $l_{2+}$).  The practical side is that it will help to keep us from overfitting the data, where our smooth function might get too wiggly.  As $\lambda \rightarrow \infty$, the result is a linear fit because any wiggliness will add too much to the loss function. As $\lambda \rightarrow 0$, we have the opposite effect, where any wiggliness is incorporated into the model.
...
If you want a more statistical check, see `?gam.check` and `?choose.k`.

Also note the following from the latter help file:
...
First we'll start with the basic GAM plot. First we get the term for year, i.e. the linear combination of the basis functions for year.  The <span class="pack">mgcv</span> package provides this for you via the predict function.  Next we need the partial residuals, which are just the basic residuals plus the term of interest added. With the original predictor variable, we're ready to proceed.


```{r termplot_gam, fig.show='hold', dev='svglite'}
...
Now for <span class="pack">visreg</span>. It uses the predict function also, but gets a standard prediction while holding the other variables at key values (which you can manipulate). By default these key values are at the median for numeric variables, and most common category for categorical variables.

```{r termplot_visreg, fig.show='hold', dev='svglite'}
