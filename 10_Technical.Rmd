...
Workshops see all types of folks at different levels. This is for some of the more technically inclined, though you'll find more context and detail in [my doc](https://m-clark.github.io/docs/GAM.html) and Wood (2006), from which a good chunk of this section is taken more or less directly from.

## GAM Introduction

A GAM is a GLM whose linear predictor includes a sum of smooth functions of covariates. With link function $g(.)$, model matrix $X$ of $n$ rows and $p$ covariates (plus a column for the intercept), a vector of $p$ coefficients $\beta$, and observations $i$, we can write a GLM as follows:
...
Where $Z$ represents the basis functions of some subset of $X$ covariates. Thus we can have any number of smooth terms, possibly of different bases, and even combinations of them.

Finally we can depict structured additive regression, or STAR, model as a GAM + random effects ($\Upsilon$) + spatial effects ($\Xi$) + other fun stuff.
...
The following will demonstrate a polynomial spline by hand. The visual display is based on a depiction in Fahrmeier et al. (2013).
...
It might look complicated, but note that there is nothing particularly special about the model itself. It is just a standard linear regression model when everything is said and done.
...
But before getting too far let's see it in action. Here our polynomial *spline* will be done with degree $l$ equal to 1, which means that we are just fitting a linear regression between knots.
...
One of the more common approaches with GAMs uses a cubic spline fit. So we'll change our polynomial degree from 1 to 3.
