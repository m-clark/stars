...
The R^2^ for this is `r broom::glance(mod_poly)$r.s %>% round(2)`, that's great! But look closely and you might notice that we aren't capturing the lower tail of this target at all, and not doing so great for observations that are high on both variables.  Here's what the data and fit would look like if we extend the data based on the underlying true function, and things only get worse.
