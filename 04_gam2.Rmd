...
Let's look at some more interesting data. For the following example we'll use the iris data set.  Just kidding! We'll look at some data regarding the Internet Movie Database (IMDB) from the <span class="pack">ggplot2movies</span> package (you'll need to install it, the data object is called 'movies').  For information, type `r ?movies` after loading the package.
...
movies_yearly = movies %>% 
  filter(!is.na(budget_cpi), budget_cpi>1e3) %>% #length>60, 
  mutate(Action=factor(Action, labels=c('Other', 'Action'))) %>% 
  group_by(year, Action) %>% 
  summarise(Budget = mean(budget_cpi),
            Rating = mean(rating),
            Votes = mean(votes)) %>% 
  ungroup()
# movies_yearly %>% 
...
<br>
Let's examine the relationship between the number of votes as a proxy for popularity, and it's rating.  For a little bit more manageable data, we'll use yearly means, and only movies that had a known budget.  In addition, I've grouped them by whether they are an action movie or not.
<br>

```{r}
movies_yearly = movies %>% 
  filter(!is.na(budget_cpi)) %>% 
  mutate(Action=factor(Action, labels=c('Other', 'Action'))) %>% 
  group_by(year, Action) %>% 
  summarise_at(.cols=vars(budget_cpi, rating, votes, length), .funs= function(x) round(mean(x), digits=1)) %>% 
  ungroup()
...
movies_yearly %>% 
  plot_ly() %>% 
  add_markers(~year, ~budget_cpi, color=~Action) %>% 
  add_lines(~year, ~budget_cpi, color=~Action)
```
