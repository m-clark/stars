...
There are many options available in <span class="pack"> + age_cen</span> beyond cubic splines, including thin plate, duchon, p-splines and more, including ones for cyclical effects, geographical, and beyond.  See `?smooth.terms` for more information.
...
mod_gamm = gam(Reaction ~ Days + s(Subject, bs='re') + s(Days, Subject, bs='re'), data=sleepstudy, 
               method='REML')
...
The incorporation of additive, mixed, and spatial regression models gets us to STARs or, <span class="">structured additive regression</span> models.  This is an extremely general and powerful modeling tool that can take you very, very far.
...
In addition to distribution extensions that could also be incorporated with random effects, providing modeling capabilities not generally offered in mixed model pacakges, mgcv also allows for correlated residuals structure as with nlme, but for the generalized case.  This is especially of interest in the longitudinal case, where one expects residuals for observations closer in time to be more correlated.
