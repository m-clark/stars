...
```{r plotlmfit, echo=FALSE, dev='png'}
augment(mod) %>% 
...
```{r covariatePlot, echo=FALSE, dev='png'}
# p =
  dat %>% 
  select(x1, x2, y) %>% 
...
```{r loglmdiag, echo=FALSE, dev='png'}
par(mfrow=c(1,2))
...
```{r plotloglmfit, echo=FALSE, dev='png'}
augment(modlog) %>% 
